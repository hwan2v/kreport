...
class OpenSearchSearcher(IndexPort):
    """NormalizedChunk들을 OpenSearch에 bulk 적재하는 어댑터."""
    def __init__(self, client: OpenSearch, alias_name: str) -> None:
        self.client = client
        self.alias_name = alias_name

    def search(self, alias_name: str, query: str, size: int = 3) -> [NormalizedChunk]:
        body = self._build_query(query, size)
        print(body)
        return self.client.search(index=alias_name, body=body)

    def _build_query(self, query: str, size: int = 3) -> dict:
        """
